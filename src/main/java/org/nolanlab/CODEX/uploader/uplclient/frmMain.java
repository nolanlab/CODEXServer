...
import org.nolanlab.CODEX.utils.microscope.Microscope;
import org.nolanlab.CODEX.utils.microscope.MicroscopeFactory;
            ...
            try {
                ...
                for (int reg : exp.getRegIdx()) {
                    for (int tile = 1; tile <= exp.getRegion_height() * exp.getRegion_width(); tile++) {
                        //serverconfig
                        copyFileToServer(experimentView.getPath(), new File(File.separator + File.separator + experimentView.getComputerName() + File.separator + upC + File.separator + exp.getUserName() + File.separator + exp.getName()), reg, tile, exp);
                        UploaderClient rnDiffta = new UploaderClient("http://localhost:4567", "runDriffta?user="+exp.getUserName()+"&exp="+exp.getName()+"&reg="+String.valueOf(reg)+"&tile="+String.valueOf(tile));
